...
def load_mesh_state(engine, f5_intelligence, p5_intelligence):
    ...
    payload = {
        "mission_control": {
            "cycle_id": cycle_id,
            "timestamp": current_time_est.strftime("%Y-%m-%d %H:%M:%S %Z"),
    ...
    # We print the raw payload instead of hitting Supabase to avoid missing column errors right now
    print("=== BEGIN PAYLOAD ===")
    print(json.dumps(payload, cls=DataMeshJSONEncoder, indent=4))
    print("=== END PAYLOAD ===")
...
if __name__ == "__main__":
    try:
        ...
        f5_data, p5_data = extract_raw_data(engine)
        
        f5_intel = execute_f5_traps(f5_data)
        p5_intel = execute_p5_traps(p5_data)
        
        load_mesh_state(engine, f5_intel, p5_intel)
        
    except Exception as e:
        print(f"\n❌ [CRITICAL SCRIPT FAILURE]: {str(e)}", file=sys.stderr)
        sys.exit(1)
